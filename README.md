...
USB interface presence is refreshed every 1.5 seconds. Cached identities are discarded when an interface disappears, so reconnecting the same device on the same COM number triggers identification again. When a possible competing master interface disappears while the USB-COMi-TB remains connected, the transition automatically triggers one bounded direct fingerprint attempt; a manual Refresh is not required. Because USB presence alone cannot otherwise establish whether another device is acting as the RS-485 master, active Modbus transmission remains separate from passive interface discovery. A failed scan leaves the adapter highlighted yellow and explains that either nothing is connected or another master may be active; it does not identify that master or report the downstream instrument as absent.
