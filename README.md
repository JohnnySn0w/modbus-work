...
Because USB presence alone cannot establish whether another device is acting as the RS-485 master, discovery performs only one bounded direct fingerprint attempt per adapter connection or manual Refresh. A failed scan leaves the adapter highlighted yellow and explains that either nothing is connected or another master may be active; it does not identify that master or report the downstream instrument as absent.
