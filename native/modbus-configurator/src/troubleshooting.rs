...
pub fn show(ui: &mut egui::Ui, reference: &Reference, selected: &mut Option<String>) {
    ...
    if let Some(device) = selected.as_ref().and_then(|key| reference.devices.get(key)) {
        ...
        if matches!(device.key.as_str(), "adapter" | "bridge" | "synetica_usb") {
            ui.add_space(16.0);
            ui.strong("Modbus Bridge hardware switch and adapter access");
            ui.add(egui::Label::new("The Modbus Bridge can be switched off with its hardware switch while external power remains connected. Turning automatic polling off in Settings does not establish that the Modbus Bridge has stopped driving RS-485. Use only one active master on the shared bus.").wrap());
        }
    } else {
        for (title, text) in [
            (
            ...
            (
                ...
                "The adapter is visible even when another Modbus master may be active. Switch the Modbus Bridge off using its hardware switch before direct adapter reads. External power can remain connected; its USB interface goes away when switched off.",
            ),
            (
                "A command is queued or taking longer than expected",
                "Manual actions wait for the current automatic read to finish. The bottom status bar shows queued work and progress. Cancel queued removes only the waiting action. Avoid unplugging equipment during programming; a failed or interrupted write requires reviewing the recovered Modbus Bridge configuration before another write.",
            ),
            (
                "Configuration does not match the attached sensor",
                "The Modbus Bridge point table describes the requested registers. A selected device type does not verify the physical sensor model. Check the sensor model, slave address, baud rate, parity and wiring. In Configuration, select the matching profile or configuration file and review the differences before using Program Modbus Bridge.",
            ),
            (
                "Backup, load or programming failed",
                "Use Configuration to load or save a configuration file, retrieve a backup, or program the Modbus Bridge. Programming requires a successful backup and verifies the resulting point table. Check the reported error and folder permissions. After an uncertain write, inspect the recovered console and export before retrying; do not assume the change completed.",
            ),
            (
...
fn guidance(line: &str) -> String {
    if line.contains("Power down or isolate the Modbus Bridge master") {
        return "Ensure the Modbus Bridge is not an active master before direct polling. See the hardware-switch guidance below.".into();
    }
