//! Automatic register charts, separated by source and complete point definition.
use eframe::egui;
...
impl HistoryView {
    pub fn show(&mut self, ui: &mut egui::Ui, history: &History) {
        ...
        let series = group_samples(history);
        if series.is_empty() {
            ui.label("Waiting for live readings");
        } else {
            ui.label(format!("{} register graphs", series.len()));
        }
        for ((source, definition), samples) in series {
            ui.push_id((source, definition), |ui| {
                ui.separator();
                ui.heading(series_label(source, definition));
                crate::brand::collapsing(ui, "Point definition", |ui| {
                    ui.label(source);
                    ui.label(definition);
                });
                show_series(ui, definition, &samples);
            });
        }
    }
}

/// Group once per frame; never combine different slaves, sources or encodings.
fn group_samples(
    history: &History,
) -> std::collections::BTreeMap<(&str, &str), Vec<&modbus_configurator::history::Sample>> {
    let mut series = std::collections::BTreeMap::<_, Vec<_>>::new();
    for sample in &history.samples {
        series
            .entry((sample.source.as_str(), sample.definition.as_str()))
            .or_default()
            .push(sample);
    }
    series
...
/// Render native values and preserve failed reads as gaps.
fn show_series(
    ui: &mut egui::Ui,
    definition: &str,
    samples: &[&modbus_configurator::history::Sample],
) {
    ...
    if values.is_empty() {
        ...
        for sample in samples {
            if let Some(v) = sample.value {
                ...
                {
                    ui.painter()
                        .line_segment([old, position], egui::Stroke::new(1.5, crate::brand::CYAN));
                }
                ui.painter()
    ...
    if let Some(last) = samples.last() {
        ui.label(format!("Last sample: {} · {}", last.timestamp, last.status));
    }
    ui.add_space(12.0);
    crate::brand::collapsing(ui, "Latest samples", |ui| {
...
fn series_label(source: &str, definition: &str) -> String {
    ...
    let fields: Vec<_> = details.split(" / ").collect();
    let address = if fields.len() == 8 {
        format!(
            "Slave {} · Register {} ({})",
        ...
        details
            .replace("address ", "Slave ")
            .replace("PDU ", "Register ")
    };
    format!("{address} · {name} [{unit}] · USB {identity}")
}
