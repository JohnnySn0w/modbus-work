...
def extend(document, root):
    ...
    devices['hmd65']['help_setup']=['Suggested: confirm Modbus RTU mode, slave address, baud and parity from the HMD65 DIP settings (Vaisala M212264EN-B).','Suggested: prefer the documented float32 bank; metric starts at 1 and non-metric starts at 129. Addresses sent on the wire are one lower.']
    devices['hmd65']['help_troubleshooting']=['Suggested: check device and measurement status bitfields; error code occupies 514–515.','Suggested: confirm unit bank and word order against a known measurement before programming an E5 bridge profile.']
    devices['wattnode']['help_setup']=['Suggested: confirm the WND-M1-MB label and communication settings using WND-M1-MB-Ref-1.10.','Suggested: check CT primary ratings and electrical service mapping before interpreting current and power.']
    devices['wattnode']['help_troubleshooting']=['Suggested: compare native float readings and status with the WND-M1-MB reference manual.','Suggested: preserve existing calibration values; do not treat CurrentIntScale as milliamps.']
    ati=copy.deepcopy(devices['hmd65']);ati.update(key='ati-f12',name='ATI F12 — PAA',subtitle='Toxic gas transmitter · PAA sensor module',kind='sensor',status='Ready to test',description='Modbus gas measurements and transmitter status. PAA is the initial sensor variant; other modules use the same transmitter register interface with module-specific units and range.',artifact='artifacts/bridge-config/ati-badger-f12-d12-documentation-test.tsv',help_setup=['TBD'],help_troubleshooting=['TBD'])
    ...
    for i,r in enumerate(metric):
        r=copy.deepcopy(r);r['name']+=' (non-metric)';r['logical']='–'.join(str(int(x)+128) for x in r['logical'].split('–'));r['pdu']='–'.join(str(int(x)+128) for x in r['pdu'].split('–'));r['unit']=['%RH','°F','°F','°F','gr/ft³','gr/lb','°F','Btu/lb'][i];r['description']='Native non-metric IEEE-754 float32 bank (Vaisala M212264EN-B).';regs['hmd65'].append(r)
