...
- One active master on shared RS-485. Modbus Bridge hardware switch-off removes its USB interface while external power may remain.
- Persistent Modbus Bridge serial ownership and bounded recovery; never blindly resend mutating commands.
...
- Modbus Bridge point-table backup is not full configuration/credential/firmware backup.
