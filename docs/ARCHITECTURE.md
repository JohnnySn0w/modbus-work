...
The native entry point owns startup and shared state. Event handling, command scheduling, configuration files, preferences, and rendering are in app_events, app_commands, app_configuration, app_settings, and app_frame. Bundled reference extraction is in reference_files. Device pages and E5 report parsing have dedicated submodules. See [code style](CODE-STYLE.md) for formatting and documentation expectations.
